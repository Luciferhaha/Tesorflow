	...
	def __init__(self, config, w2i_target, useTeacherForcing=True, useAttention=True, useBeamSearch=1):
		...
		with tf.variable_scope("encoder"):
			...
			with tf.variable_scope("gru_cell"):
				encoder_cell = tf.keras.layers.GRUCell(config.hidden_dim)
			
			((encoder_fw_outputs, encoder_bw_outputs), (encoder_fw_final_state, encoder_bw_final_state)) = tf.keras.layers.Bidirectional(cell_fw=encoder_cell, cell_bw=encoder_cell, inputs=encoder_inputs_embedded, sequence_length=self.seq_inputs_length, dtype=tf.float32, time_major=False)
